/*{
  "CATEGORIES": ["Generator", "Art Movement"],
  "DESCRIPTION": "Joan Miró surrealist dreamscape — biomorphic creatures, primary-colour blobs, and bold sinuous black lines on a warm two-tone ground. After The Harlequin's Carnival (1924), Dutch Interior I (1928), The Hunter — Catalan Landscape (1923), and Constellations (1941).",
  "INPUTS": [
...
// Organic blob: smooth union of main body + two satellite lobes (static).
float sdOrganic(vec2 p, float r, float fi) {
    float d  = sdCircle(p, r * 0.72);
    float a1 = h11(fi * 11.7) * 6.2832;
    float a2 = h11(fi * 31.1) * 6.2832;
...
void main() {
    ...
    // Static wavy horizon
    float split = groundSplit + 0.018 * sin(uv.x * 5.71);
    ...
    for (int i = 0; i < 18; i++) {
        ...
        vec2  ctr = vec2(0.08 + h11(fi * 1.31) * 0.84,
                         0.08 + h11(fi * 2.73) * 0.84);
        ctr.x *= asp;
        float r   = 0.035 + h11(fi * 5.53) * 0.065;
        float sd  = sdOrganic(P - ctr, r, fi);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        vec2  ctr = vec2(0.08 + h11(fi * 11.37) * 0.84,
    ...
    for (int i = 0; i < 8; i++) {
        ...
        vec2  ctr = vec2(0.06 + h11(fi * 23.13) * 0.88,
                         0.06 + h11(fi * 29.71) * 0.88);
        ctr.x *= asp;

        float rot = h11(fi * 37.7) * 6.2832;
    ...
    // ── Crescent moon ────────────────────────────────────────────────────────
    {
        vec2  moonCtr = vec2(0.76 * asp, 0.70);
        float mSD = sdCrescent(P - moonCtr, 0.052, vec2(0.024, 0.0), 0.046);
    ...
    for (int k = 0; k < 6; k++) {
        ...
        float amp     = 0.06 + h11(fk * 43.71) * 0.09;
        float frq     = 0.8  + h11(fk * 47.13) * 1.2;
        float ph      = fk * 2.1;   // static phase from seed only
    ...
    for (int k = 0; k < 10; k++) {
        ...
        float ang = h11(fk * 83.71) * 6.2832;   // static angle
    ...
    for (int k = 0; k < 14; k++) {
        ...
        vec2  ctr = vec2(h11(fk * 89.17) * asp, h11(fk * 97.33));  // static
